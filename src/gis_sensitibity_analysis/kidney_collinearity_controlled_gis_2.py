...
for j in range(len(most_relevant_genes)):
    ...
    for k in range(100):
        ...
        penalties = get_GIS_scores_lasso_func([], rpm_log_new.columns, k=1, v=1, k=1, v=1, go=True, reactome=False,
        ...
        penalties[-10:] = np.linspace(0.95,  np.max(penalties), 10)
        print(penalties[-10:])
    ...
    save_dict(dct, os.path.join(dire_r, 'rounds_'+ most_relevant_genes[j] + '_2'))
    save_dict(dct_f,os.path.join(dire_r,'rounds_features_'+ most_relevant_genes[j] + '_2'))