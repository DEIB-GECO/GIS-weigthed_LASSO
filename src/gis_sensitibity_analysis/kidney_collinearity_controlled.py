...
for j in range(len(most_relevant_genes)):
    ...
    for k in range(100):
        ...
        # visualize resuts
    #save_dict(dct,'rounds_'+ most_relevant_genes[j] + '_st')

    save_dict(dct, os.path.join(dire_r,'rounds_'+ most_relevant_genes[j] + '_st'))
    save_dict(dct_f,os.path.join(dire_r,'rounds_features_'+ most_relevant_genes[j] + '_st'))
