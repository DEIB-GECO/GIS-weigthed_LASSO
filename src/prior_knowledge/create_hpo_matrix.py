...
def load_annotations_file(filename):    
    with open(filename) as f:
        ann = {}
        for line in f:
            entrez, term = line.strip().split(' ')
            if entrez in ann:
                ann[entrez].append(term)
            else:
                ann[entrez] = []
                ann[entrez].append(term)
...
class hpo_matrix:
    ...
    def __init__(self, hpo_file, alt_file, ids_file, genes_list):
        self.hpo_file = hpo_file
        self.alt_file = alt_file
        self.ids_file = ids_file
        self.genes_list = genes_list

    def build_matrix(self, gene_hpo_file):

        d_hpo_anns = load_dict(self.hpo_file)
        #print(len(d_hpo_anns))
        d_hpo_alts = load_dict(self.alt_file)
        d_gene_anns = load_annotations_file(gene_hpo_file)
        hpo_genes = d_gene_anns.keys()
        hpo_terms = d_hpo_anns.keys()
        gene_index, go_index=  get_indexes(hpo_genes,  hpo_terms)
        ...
        for g in tqdm.tqdm(d_gene_anns):
            for go in d_gene_anns[g]:
                if go in go_index:
                    ...
                    if go in d_hpo_alts:
                        indice_go = go_index[d_hpo_alts[go]]
                        new_go = d_hpo_alts[go]
               
                matrix[gene_index[g], indice_go] = 1.
                for anc in d_hpo_anns[new_go]:
                    if anc in d_hpo_anns:
                        indice_go = go_index[anc]
                    else:
                        if anc in d_hpo_alts:
                            indice_go = go_index[d_hpo_alts[anc]]
                    matrix[gene_index[g], indice_go] = 1.
    ...
    def build_weighted_matrix(self, gene_hpo_file, IC_scores_file):
        ...
        #print(scores.head())
        scores_sorted = {k:scores.loc[k] for k in d_hpo_anns}
    ...
    def get_significant_terms(self, genes_subset, universe='all', alternative = 'two-sided',
        ...
        anns_u = np.unique(all_anns)
        anns_u = [x for x in anns_u if x.startswith('HP')]
        test = {}
