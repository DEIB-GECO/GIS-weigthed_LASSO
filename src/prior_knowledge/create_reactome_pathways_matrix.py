...
class Reactome_pathways_matrix:
    def __init__(self, filename, ids_file, genes_list):
    ...
    def build_empty_matrix(self):
        ...
        pathways = np.unique(self.df[self.df.columns[0]])
    ...
    def build_matrix(self):
        ...
        for gene in tqdm.tqdm(self.pm.index):
            paths = self.df.loc[gene,self.df.columns[0]]
    ...
    def get_gene_ids_from_sys(self, genes_list, verbose=True):
        ...
        if verbose:
            print('Number of selected genes with Reactome annotations:' ,len(genes_sys))
        else:
            print('The total number of genes with Reactome annotations in the dataset:' ,len(genes_sys))
